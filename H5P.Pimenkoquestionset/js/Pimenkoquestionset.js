 ...
 *
 * Events provided:
 * - h5pPimenkoquestionsetFinished: Triggered when a question is finished. (User presses Finish-button)
 *
 * @param {Array} options
 * @param {int} contentId
 * @param {Object} contentData
 * @returns {H5P.Pimenkoquestionset} Instance
 */
H5P.Pimenkoquestionset = function(options, contentId, contentData) {
    if (!(this instanceof H5P.Pimenkoquestionset)) {
        return new H5P.Pimenkoquestionset(options, contentId, contentData);
    }
    ...
    var defaults = {
        ...
        endGame: {
            ...
            continueRedirectUrl: 'http:\/\/www.example.com',
    ...
    var attempts = urlParams.get("attempts");
    ...
    var texttemplate =
        ...
        '<div class="pimenko-questionset<% if (introPage.showIntroPage) { %> hidden<% } %>">' +
    ...
    && (params.attemptsNumber === 0 || attempts < params.attemptsNumber) ?
        '    <button type="button" class="h5p-joubelui-button h5p-button qs-retrybutton"><%= retryButtonText %></button>' :
        '';

    const continueButtonTemplate = params.endGame.showContinueButton  ?
        '    <button type="button" class="h5p-joubelui-button h5p-button qs-continuebutton"><%= continueButtonText %></button>' :
        '';
    ...
    var _displayEndGame = function() {
        ...
        if (rendered) {
            $myDom.children().hide().filter('.pimenko-questionset-results').show();
            self.trigger('resize');
            return;
        }
        //Remove old score screen.
        $myDom.children().hide().filter('.pimenko-questionset-results').remove();
        rendered = true;
        ...
        var displayResults = function() {
            ...
            if (params.attemptsNumber !== 0 && attempts >= params.attemptsNumber) {
                $('.qs-retrybutton', $myDom).remove();
            }

            if (params.endGame.showResultPage) {
                hookUpButton('.qs-solutionbutton', function() {
                    showSolutions();
                    $myDom.children().hide().filter('.pimenko-questionset').show();
                    _showQuestion(params.initialQuestion);
                });
                hookUpButton('.qs-continuebutton', function() {
                    window.top.location.href = params.endGame.continueRedirectUrl;
                });
                hookUpButton('.qs-retrybutton', function() {
                    ...
                    if ($intro.length) {
                        ...
                        // Show first question
                        $('.pimenko-questionset', $myDom).show();
                        _showQuestion(params.initialQuestion);
                    }
                });
                ...
                $('.qs-solutionbutton, .qs-retrybutton, .qs-continuebutton, .feedback-section', $myDom).remove();
            }
    ...
    this.attach = function(target) {
        ...
        if (params.attemptsNumber !== 0 && attempts >= params.attemptsNumber) {
          $template = '<div class="noattempts">'+params.noAttemptsText+'<br><button type="button" class="h5p-joubelui-button h5p-button qs-continuebutton">'+params.endGame.continueButtonText+'</button>' + '</div>';
        }
        ...
        $('.pimenko-questionset', $myDom).addClass('started');
        ...
        if (params.attemptsNumber !== 0 && attempts >= params.attemptsNumber) {
            $('.qs-continuebutton', $myDom).click(function() {
                window.top.location.href = params.endGame.continueRedirectUrl;
            });
        }
    ...
    // Get current score for Pimenkoquestionset.
    ...
    // Get total score possible for Pimenkoquestionset.
...
H5P.Pimenkoquestionset.prototype = Object.create(H5P.EventDispatcher.prototype);
H5P.Pimenkoquestionset.prototype.constructor = H5P.Pimenkoquestionset;